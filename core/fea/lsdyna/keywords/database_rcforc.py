...
class DatabaseRcforc(ABC):
    """
    user define DatabaseRcforc() object
    input 1 parameters:
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.name == "default":
            self.name = f"database_rcforc_{self.uid}"

    def __repr__(self):
        if self.name == "default":
            return (f"database_rcforc id: {self.uid}, "
                    f"dt: {self.dt}, "
            ...
            return (f"database_rcforc name: {self.name}"
                    f"database_rcforc id: {self.uid}, "
                    f"dt: {self.dt}, "
                    f"binary: {self.binary}, "
                    f"lcur: {self.lcur}, "
                    f"ioopt: {self.ioopt}\n")